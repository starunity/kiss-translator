...
export class YouTubeSubtitleList {
  ...
  constructor(videoElement) {
    ...
    this.subActionBar = null; // 字幕操作栏（遮蔽控件所在）
  ...
  _renderTabsAndStructure() {
    ...
    //    1.1 字幕操作工具条 (固定在顶部，左侧为下载按钮组，右侧为遮蔽控件)
    this.subActionBar = document.createElement("div");
    this.subActionBar.style.cssText = `padding: 10px 16px; border-bottom: 1px solid var(--kt-divider); display: flex; justify-content: space-between; align-items: center; gap: 8px; flex-shrink: 0;`;

    // 下载按钮分组容器
    const subDownloadGroup = document.createElement("div");
    subDownloadGroup.style.cssText = `display: flex; gap: 8px; align-items: center;`;
    ...
    subDownloadGroup.append(downloadBtn, downloadRawBtn);
    this.subActionBar.appendChild(subDownloadGroup);

    // 挂载遮蔽模式切换按钮 (DomManager 将其渲染到下载按钮组右侧)
  ...
  _createSubtitleListItem(sub, index) {
    ...
    // 动作绑定：点击字幕整行，直接同步将视频进度拉至字幕开始时间轴，带有字幕索引避免边界时间偏差
